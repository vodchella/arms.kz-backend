...
class CurrentUser:
    @staticmethod
    async def get(token: HTTPAuthorizationCredentials = Depends(auth_scheme)) -> User:
        return await _get_current_user(token.credentials, 'a')

    @staticmethod
    async def get_by_refresh_token(token: HTTPAuthorizationCredentials = Depends(auth_scheme)) -> User:
